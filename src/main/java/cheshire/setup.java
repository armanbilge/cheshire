...
class setup {
	...
	private static int __sys_io_uring_setup(int entries, MemorySegment p) {
		try {
			...
			int ret = (int) syscallHandle.invokeExact(constants.__NR_io_uring_setup, entries, p);
			if (ret < 0) {
	...
	private static int __sys_close(int fd) {
		try {
			MethodHandle syscallHandle = linker.downcallHandle(
	...
	private static int __sys_munmap(long addr, long length) {
		// TODO
		return 0;
	};

	private static MemorySegment __sys_mmap(MemorySegment addr, long length, int prot, int flags,
			int fd, long offset) {
		try {
			// TODO
			MethodHandle syscallHandle = linker.downcallHandle(
			...
			MemorySegment ret;
			ret = (MemorySegment) syscallHandle.invokeExact(constants.__NR_io_uring_setup, fd);
	...
	private static void io_uring_setup_ring_pointers(MemorySegment p, MemorySegment sq, MemorySegment cq) {
		...
		io_uring_sq.kringEntrieskVarHandle.set(sq, sq_ring_ptr + (int) io_sqring_offsets.ringEntriesVarHandle.get(sq_off));
		...
		io_uring_cq.kringEntrieskVarHandle.set(cq, cq_ring_ptr + (int) io_cqring_offsets.ringEntriesVarHandle.get(cq_off));
		...
		io_uring_sq.ringEntriesVarHandle.set(sq, io_uring_sq.kringEntrieskVarHandle.get(sq));
		io_uring_cq.ringMaskVarHandle.set(cq, io_uring_cq.kringMaskVarHandle.get(cq));
		io_uring_cq.ringEntriesVarHandle.set(cq, io_uring_cq.kringEntrieskVarHandle.get(cq));
	};
	...
	private static int io_uring_mmap(int fd, MemorySegment p, MemorySegment sq, MemorySegment cq) {
		...
		io_uring_sq.ringPtrVarHandle.set(sq,
				__sys_mmap(MemorySegment.NULL, (long) io_uring_sq.ringSzVarHandle.get(sq),
		...
		if ((features & constants.IORING_FEAT_SINGLE_MMAP) != 0) {
			...
			io_uring_cq.ringPtrVarHandle.set(sq,
					__sys_mmap(MemorySegment.NULL, (long) io_uring_cq.ringSzVarHandle.get(cq),
							constants.PROT_READ | constants.PROT_WRITE,
		...
		io_uring_sq.sqesVarHandle.set(sq,
				__sys_mmap(MemorySegment.NULL, size * ((long) io_uring_params.sqEntriesVarHandle.get(p)),
	...
	private static void io_uring_unmap_rings(MemorySegment sq, MemorySegment cq) {
		...
		if (sqRingSz != 0)
			__sys_munmap(sqRingPtr, sqRingSz);
		if (cqRingPtr != 0 && cqRingSz != 0 && cqRingPtr != sqRingPtr)
			__sys_munmap(cqRingPtr, cqRingSz);
	}
	...
	private static int io_uring_alloc_huge(int entries, MemorySegment p, MemorySegment sq, MemorySegment cq,
		...
		try (Arena memorySession = Arena.ofConfined()) {
			...
			MemorySegment sq_entries_segment = memorySession.allocate(ValueLayout.JAVA_INT);
			MemorySegment cq_entries_segment = memorySession.allocate(ValueLayout.JAVA_INT);
			long ring_mem, sqes_mem = 0;
			long mem_used = 0;
			MemorySegment ptr = memorySession.allocate(ValueLayout.ADDRESS);
			...
			if (buf != null) {
				...
				ptr.set(ValueLayout.ADDRESS, 0L, buf);
			} else {
				int map_hugetlb = 0;
				...
				ptr.set(ValueLayout.ADDRESS, 0L,
						__sys_mmap(null, buf_size, constants.PROT_READ | constants.PROT_WRITE,
	...
	private static int __io_uring_queue_init_params(int entries, MemorySegment ring, MemorySegment p,
		...
		if ((flags & constants.IORING_SETUP_NO_MMAP) != 0) {
			MemorySegment sq1 = ring.asSlice(io_uring.layout.byteOffset(PathElement.groupElement("sq")), io_uring_sq.layout);
			MemorySegment cq1 = ring.asSlice(io_uring.layout.byteOffset(PathElement.groupElement("cq")), io_uring_cq.layout);
			ret = io_uring_alloc_huge(entries, p, sq1, cq1, buf, buf_size);
			if (ret < 0) {
				return ret;
			}
			if (buf != null) {
		...
		if (fd < 0) {
			char int_flags = (char) io_uring.intFlagsVarHandle.get(ring);
			if (((flags & constants.IORING_SETUP_NO_MMAP) != 0) && ((int_flags & constants.INT_FLAG_APP_MEM) == 0)) {
		...
		if (((flags & constants.IORING_SETUP_NO_MMAP) == 0)) {
			...
			MemorySegment sq3 = ring.asSlice(io_uring.layout.byteOffset(PathElement.groupElement("sq")), io_uring_sq.layout);
			MemorySegment cq3 = ring.asSlice(io_uring.layout.byteOffset(PathElement.groupElement("cq")), io_uring_cq.layout);
		...
		int sq_entries = (int) io_uring_sq.ringEntriesVarHandle.get(sq4);

		if ((flags & constants.IORING_SETUP_NO_SQARRAY) == 0) {
			MemorySegment sq_array = sq4.asSlice(io_uring_sq.layout.byteOffset(PathElement.groupElement("array")),
					ValueLayout.ADDRESS);
			for (int index = 0; index < sq_entries; index++) {
				sq_array.setAtIndex(ValueLayout.JAVA_INT, index, index);
			}
		}
		io_uring.featuresVarHandle.set(ring, (int) io_uring_params.featuresVarHandle.get(p));
		io_uring.flagsVarHandle.set(ring, (int) io_uring_params.flagsVarHandle.get(p));
	...
	public static int io_uring_queue_init_params(int entries, MemorySegment ring, MemorySegment p) {
		int ret = io_uring_queue_init_try_nosqarr(entries, ring, p, null, 0L);
		return ret >= 0 ? 0 : ret;
	};
}
